...
const id_inet = '8c0b7beda77fa724315d693d033e3a14';
...
const jp_yomiuri_new = '5744a9fea3c9caab46839d39c9f08ec1';
const jp_yomiuri_top = '0508d3a3e46f470b6552ae0b92d65116';
const jp_yomiuri_rank= '03bc2ed5efac4a80e928a9280a6791e4';
...
const yomiuri_top = 'yomiuri_top';
const yomiuri_rank = 'yomiuri_rank';
...
$(window).load(function(){
	...
	$('div[id="top"] div[id="region_group"] input[name="radioreg"]').live("change", function () {
	...
	$('div[id="news_list"]').live("pageshow", function () {
		...
		} else if(current_page == det_inet){
			common.loadPipes(id_inet, desc, callback_a);
		...
		} else if(current_page == yomiuri_top){
			common.loadPipes(jp_yomiuri_top, desc, callback_a);
		} else if(current_page == yomiuri_rank){
			common.loadPipes(jp_yomiuri_rank, desc, callback_a);
...
var common = {
		loadPipes : function(idnya, desc, callback, query, lang){
			...
			$.getJSON(str_url,
					  ...
					  function(data) {
						  iter = 0;
				          content_desc = [];
						  if(idnya == id_sepak || idnya == id_news || idnya == id_inet){
						  ...
						  } else if(idnya == jp_yomiuri_new || idnya == jp_yomiuri_top || idnya == jp_yomiuri_rank){
		...
		viewArticle : function(data, content){
			...
			else if(current_page == yomiuri_new || current_page == yomiuri_top || current_page == yomiuri_rank)
		...
		scanningPage : function(html_array, berita_type){
			...
			do{
				if(is_berita_content){
					if(is_get_content){
						var str_line =  $.trim(html_array[i]);
						if((str_line.indexOf(akhir_berita[0])) > -1 || (str_line.indexOf(akhir_berita[0])) > -1)
							isdata_load = true;
